...
function mixColors(first, second, weight) {
    ...
    const mix = (left, right) => Math.round(left * (1 - clampWeight) + right * clampWeight);
    const toHex = (value) => value.toString(16).padStart(2, "0");
    return `#${toHex(mix(a.r, b.r))}${toHex(mix(a.g, b.g))}${toHex(mix(a.b, b.b))}`;
}
function getContributionShade(levels, contributionStrength, maxContributionStrength) {
    if (levels.length === 0) {
        return "#39d353";
    }
...
function getCellColors(palette, activeContribution, powerCell, contributionStrength, maxContributionStrength) {
    ...
    const contributionShade = getContributionShade(palette.contributionLevels, contributionStrength, maxContributionStrength);
...
function pacmanPath(radius, mouth) {
    const start = mouth;
...
export function renderGameSvg(simulation, config) {
    ...
    const maxContributionStrength = Math.max(1, ...mapStrength.flat().map((value) => value ?? 0));
    ...
    svg += `<desc id="desc">${escapeXml("Pac-Man traverses a GitHub contribution board while ghosts chase through the graph.")}</desc>`;
    svg += `<defs>
    <linearGradient id="bg-gradient" x1="0%" y1="0%" x2="100%" y2="100%">
      <stop offset="0%" stop-color="${mixColors(palette.background, palette.gradientTint, 0.18)}" />
      <stop offset="58%" stop-color="${palette.background}" />
      <stop offset="100%" stop-color="${mixColors(palette.background, palette.gradientDeep, 0.32)}" />
    </linearGradient>
    <linearGradient id="panel-gradient" x1="0%" y1="0%" x2="0%" y2="100%">
      <stop offset="0%" stop-color="${mixColors(palette.scorePanel, palette.gradientTint, 0.18)}" />
      <stop offset="100%" stop-color="${mixColors(palette.background, palette.gradientDeep, 0.14)}" />
    </linearGradient>
    <linearGradient id="inner-panel-gradient" x1="0%" y1="0%" x2="0%" y2="100%">
      <stop offset="0%" stop-color="${mixColors(palette.background, palette.gradientTint, 0.26)}" />
      <stop offset="100%" stop-color="${mixColors(palette.background, palette.gradientDeep, 0.14)}" />
    </linearGradient>
    ...
    <filter id="panel-shadow" x="-20%" y="-30%" width="140%" height="170%">
      <feDropShadow dx="0" dy="18" stdDeviation="20" flood-color="#000000" flood-opacity="${palette.shadowOpacity}" />
    </filter>
    ...
    svg += `<rect x="${panelX}" y="${panelY}" width="${panelWidth}" height="${panelHeight}" rx="30" fill="url(#panel-gradient)" stroke="${withAlpha(mixColors(palette.gridLine, "#ffffff", 0.18), 0.88)}" stroke-width="1.1"/>`;
    svg += `<rect x="${innerPanelX}" y="${innerPanelY}" width="${innerPanelWidth}" height="${innerPanelHeight}" rx="22" fill="url(#inner-panel-gradient)" stroke="${withAlpha(palette.gridLine, 0.5)}"/>`;
    svg += `</g>`;
    simulation.source.monthLabels.forEach((label, col) => {
        ...
        svg += `<text x="${x}" y="${top - 14}" fill="${withAlpha(palette.monthLabel, 0.96)}" font-size="12" font-weight="500" font-family="-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif">${escapeXml(label)}</text>`;
    });
    for (let row = 0; row < GRID_HEIGHT; row += 1) {
        for (let col = 0; col < GRID_WIDTH; col += 1) {
            ...
            const fillTimeline = buildTimeline(simulation.frames.map((frame) => {
                const cell = frame.grid[row][col];
                return getCellColors(palette, cell === 1 || cell === 4, cell === 4, contributionStrength, maxContributionStrength).fill;
            }));
            const strokeTimeline = buildTimeline(simulation.frames.map((frame) => {
                const cell = frame.grid[row][col];
                return getCellColors(palette, cell === 1 || cell === 4, cell === 4, contributionStrength, maxContributionStrength).stroke;
            }));
            const sheenTimeline = buildTimeline(simulation.frames.map((frame) => {
                const cell = frame.grid[row][col];
                return getCellColors(palette, cell === 1 || cell === 4, cell === 4, contributionStrength, maxContributionStrength).sheen;
            }));
            ...
            svg += `</rect>`;
            svg += `<rect x="${x + 1.2}" y="${y + 1.2}" width="${square - 2.4}" height="${Math.max(5, square * 0.28)}" rx="4" fill="${sheenTimeline.values.split(";")[0]}">`;
            ...
            svg += `</rect>`;
            const powerFrames = simulation.frames.map((frame) => frame.grid[row][col] === 4 ? "1" : "0");
            ...
            svg += `<rect x="${x + 2}" y="${y + 2}" width="${square - 4}" height="${square - 4}" rx="5" fill="none" stroke="${withAlpha(palette.powerRing, 0.68)}" stroke-width="1.6" filter="url(#pellet-glow)">`;
            svg += `<animate attributeName="stroke-opacity" dur="900ms" repeatCount="indefinite" values="0.32;0.88;0.32"/>`;
            svg += `</rect>`;
            svg += `<rect x="${x + 4.2}" y="${y + 4.2}" width="${square - 8.4}" height="${square - 8.4}" rx="4" fill="none" stroke="${withAlpha(palette.powerPellet, 0.46)}" stroke-width="0.9"/>`;
            svg += `</g>`;
        }
    }
    ...
    const mazePath = mazeSegments
        ...
        .join(" ");
    svg += `<path d="${mazePath}" fill="none" stroke="${withAlpha(palette.mazeOuter, 0.7)}" stroke-width="${Math.max(5.2, gap + 3.1)}" stroke-linecap="round" stroke-linejoin="round"/>`;
    svg += `<path d="${mazePath}" fill="none" stroke="${withAlpha(palette.mazeInner, 0.94)}" stroke-width="${Math.max(2.2, gap + 0.6)}" stroke-linecap="round" stroke-linejoin="round" filter="url(#maze-glow)"/>`;
    const pacmanPositions = buildTimeline(simulation.frames.map((frame) => {
        const x = left + frame.pacman.col * step + square / 2;
        const y = top + frame.pacman.row * step + square / 2;
        return `${x} ${y}`;
    }));
    const pacmanRotations = buildTimeline(simulation.frames.map((frame) => `${directionToAngle(frame.pacman.direction)} 0 0`));
    ...
    svg += `</path>`;
    svg += `<circle cx="-${radius * 0.22}" cy="-${radius * 0.24}" r="${radius * 0.24}" fill="${withAlpha("#ffffff", 0.16)}"/>`;
    svg += `<circle cx="1.4" cy="-${radius * 0.38}" r="${Math.max(1.1, radius * 0.08)}" fill="${palette.eye}"/>`;
    svg += `</g>`;
    simulation.finalState.ghosts.forEach((ghost, index) => {
        const positions = buildTimeline(simulation.frames.map((frame) => {
            const actor = frame.ghosts[index];
            const x = left + actor.col * step + square / 2;
            const y = top + actor.row * step + square / 2;
            return `${x} ${y}`;
        }));
        const fills = buildTimeline(simulation.frames.map((frame) => {
            const actor = frame.ghosts[index];
            return actor.mode === "frightened"
                ? palette.frightenedGhost
                : palette.ghosts[index % palette.ghosts.length];
        }));
        const opacity = buildTimeline(simulation.frames.map((frame) => {
            const actor = frame.ghosts[index];
        ...
        svg += `</path>`;
        svg += `<ellipse cx="-${radius * 0.18}" cy="-${radius * 0.58}" rx="${radius * 0.32}" ry="${radius * 0.12}" transform="rotate(-20)" fill="${withAlpha("#ffffff", 0.18)}"/>`;
